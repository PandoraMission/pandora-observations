...
| Target metadata (`targets.json`) | [targets.md](targets.md) | 1 |
...
- The record layer (`data/calendars/`, `data/sequences/`, `data/reports/`) is append-only and authoritative. The Parquet cache under `data/cache/` is derived and disposable; its columns are the flattened record fields and are not separately versioned.
- The `data/` directory lives in the repo root and is gitignored for now. Its root is marked by a `pandora_obs_data.json` file so the code can discover it; an explicit path always overrides discovery.
